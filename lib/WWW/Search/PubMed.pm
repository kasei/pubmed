...
use constant	ARTICLES_PER_REQUEST	=> 100;
use constant	QUERY_ARTICLE_LIST_URI	=> 'http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&retmax=100';	# term=ACTG
...
sub native_setup_search {
	...
	$self->{'_term'}		= WWW::Search::escape_query($query);
	$self->{'_retstart'}	= 0;
	return $self->_get_some_article_ids();
}

sub _get_some_article_ids {
	my $self		= shift;
	...
	if ($success) {
		...
		$self->{'_retstart'}	+= $count;
		push(@{ $self->{'_article_ids'} }, @articles);
		return 1;
	} else {
		return undef;
	}
...
sub native_retrieve_some {
	...
	my $count	= scalar(@{ $self->{'_article_ids'} || [] });
	if ($count == 0) {
		$self->_get_some_article_ids();
		if (0 == scalar(@{ $self->{'_article_ids'} || [] })) {
			$self->{'_finished'}++;
			return undef;
		}
	}
